  ...
  return {
    ...
    UserCollectionsContext: {
      Consumer: ({
        ...
        children: (value: UserCollectionsContextState) => React.ReactNode;
      }>) => <div data-testid="mock-consumer">{children(mockPagination<Collection>())}</div>,
    },
  };
});
...
jest.mock('../../../contexts/CollectionItemsContext', () => {
  ...
  return {
    ...
    CollectionItemsContext: {
      Consumer: ({
        children,
      }: PropsWithChildren<{ children: (value: any) => React.ReactNode }>) => (
...
describe('MyCollections', () => {
  ...
  it('renders collections when user has id', () => {
    renderWithProviders(<MyCollections user={testUser} />);
  ...
  test('creates collection when form is submitted', async () => {
    ...
    (CollectionManagementRequests.createCollection as jest.Mock).mockResolvedValue(
      mockNewCollection,
    );

    // Mock the addModel function
    const mockAddModel = jest.fn();
    jest.spyOn(UserCollectionsContext, 'Consumer').mockImplementation(({ children }) => (
      <div data-testid="mock-consumer-spy">
        {children(
          mockPagination<Collection>({
  ...
  test('displays error when collection creation fails', async () => {
    ...
    (CollectionManagementRequests.createCollection as jest.Mock).mockRejectedValue(
