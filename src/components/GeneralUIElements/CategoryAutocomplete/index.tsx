...
const CategoryAutocompleteContent = forwardRef<
  { clearInput: () => void },
  CategoryAutocompleteContentProps
>(
  (
    {
      onSelect,
    ...
    const prioritizedIds = useMemo(
      () => new Set((prioritizedCategories ?? []).map((cat) => cat.id)),
      [prioritizedCategories],
    );
...
const CategoryAutocomplete = forwardRef<{ clearInput: () => void }, CategoryAutocompleteProps>(
  (props, ref) => {
    return (
      <CategoriesContextProvider>
