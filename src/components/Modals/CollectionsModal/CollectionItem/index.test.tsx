...
describe('CollectionItemComponent', () => {
  ...
  it('handles adding to collection', async () => {
    (CollectionManagementRequests.createCollectionItem as jest.Mock).mockResolvedValue(
      testCollectionItem,
    );
  ...
  it('handles removing from collection', async () => {
    ...
    await waitFor(() => {
      expect(CollectionManagementRequests.removeCollectionItem).toHaveBeenCalledWith(
        testCollectionItem,
      );
    });
  ...
  it('handles adding a category', async () => {
    ...
    await waitFor(() => {
      expect(CollectionManagementRequests.createCollectionItemCategory).toHaveBeenCalledWith(100, {
        category_id: 1,
  ...
  it('handles removing a category', async () => {
    ...
    const removeCategoryButton = categoryButtons.find(
      (btn) =>
        ...
        btn.querySelector('svg')?.className.baseVal.includes('tabler-icon-trash'),
    );
