...
const handleAddToCollection = async (
  ...
  try {
    ...
    const newCollectionItem = await CollectionManagementRequests.createCollectionItem(
      collection,
      collectionItemData,
    );
...
const handleAddCategory = async (
  ...
  try {
    ...
    const newCollectionItemCategory =
      await CollectionManagementRequests.createCollectionItemCategory(
        collectionItem.id,
        categoryData,
      );
    ...
    if (collectionId && collectionItemsContext[collectionId]) {
      ...
      const itemIndex = collectionItems.findIndex(
        (item: CollectionItem) => item.id === collectionItem.id,
      );
...
const handleRemoveCategory = async (
  ...
  try {
    ...
    if (collectionId && collectionItemsContext[collectionId]) {
      ...
      const collectionItems = collectionItemsContext[collectionId].loadedData || [];
      const itemIndex = collectionItems.findIndex(
        (item: CollectionItem) => item.id === collectionItemCategory.collection_item_id,
      );
      ...
      if (itemIndex !== -1) {
        ...
        updatedCollectionItem.collection_item_categories =
          updatedCollectionItem.collection_item_categories.filter(
...
const CollectionItemComponent: React.FC<CollectionItemProps> = ({
  ...
  const collectionItem =
    collection.id && itemIsInCollection
      ? collectionItemsContext[
          collection.id as keyof typeof collectionItemsContext
        ]?.loadedData?.find((ci: CollectionItem) => ci.item_id === item.id)
      : null;
  ...
  const selectedReleases = useMemo(() => {
    ...
    const collectionState =
      collectionItemsContext[collection.id as keyof typeof collectionItemsContext];
    if (!collectionState) return [];
    // Get all items that are selected but not already in the collection
    const existingItemIds = new Set(
      collectionState.loadedData?.map((item: CollectionItem) => item.item_id) ?? [],
    );
    return Array.from(selectedItems).filter(
      (item) => item.id !== undefined && !existingItemIds.has(item.id),
    );
  ...
  const handleCategorySelect = async (category: Category) => {
    ...
    if (collectionItem && category) {
      await handleAddCategory(collectionItem, category, collectionItemsContext);
      // Clear the CategoryAutocomplete input after successful addition
      if (
        categoryAutocompleteRef.current &&
        typeof categoryAutocompleteRef.current.clearInput === 'function'
      ) {
  ...
  const handleCategoryRemove = async (category: CollectionItemCategory) => {
    await handleRemoveCategory(collection, category, collectionItemsContext);
  };
          ...
          <>
            ...
            <CategoryAutocomplete
              ...
              prioritizedCategories={
                collectionItem?.collection_item_categories?.map(
                  (cic: CollectionItemCategory) => cic.category,
                ) || []
              }
