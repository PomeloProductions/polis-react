...
const PageSettingsPanel: React.FC<PageSettingsPanelProps> = ({ page, onRefresh }) => {
  ...
  return (
    <Stack gap="md">
                      ...
                      {(dragProvided) => (
                        <Box
                          ...
                          style={{
                            ...
                            border:
                              '1px solid var(--polis-color-border, var(--mantine-color-gray-2))',
      ...
      <Modal opened={addModalOpen} onClose={() => setAddModalOpen(false)} title="Add Page">
        <Stack>
          <TextInput
            label="Page Name"
          ...
          />
          <Button onClick={handleAddPage} loading={saving} disabled={!newPageName.trim()}>
            Create Page
          </Button>
        </Stack>
      </Modal>
