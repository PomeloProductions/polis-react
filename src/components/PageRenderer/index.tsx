...
const PageRenderer: React.FC<PageRendererProps> = ({ page, userId, pageParams }) => {
    ...
    const components = (page.components ?? []).filter(
        ...
        (result: DropResult) => {
            ...
            if (srcWidget && dstWidget) {
                if (srcWidget.compId === dstWidget.compId) {
                    ...
                    const updated = moveNode(root, srcWidget.path, source.index, dstWidget.path, destination.index);
                    ...
                    if (me?.id) {
                        const srcParent = srcWidget.path.length === 0 ? root : getNodeAtPath(root, srcWidget.path);
                        if (srcParent?.id) {
                            const updatedParent = srcWidget.path.length === 0 ? updated : getNodeAtPath(updated, srcWidget.path);
                        ...
                        if (srcWidget.path.join('.') !== dstWidget.path.join('.')) {
                            const dstParent = dstWidget.path.length === 0 ? root : getNodeAtPath(root, dstWidget.path);
                            if (dstParent?.id) {
                                const updatedDstParent = dstWidget.path.length === 0 ? updated : getNodeAtPath(updated, dstWidget.path);
                                patchTodoNode(me.id, dstParent.id, dstWidget.compId, {
                                    children: (updatedDstParent.children ?? []).map((c: TodoTaskNode) => ({ id: c.id })),
                                }).catch(console.error);
                            }
                        }
                    }
                    ...
                    const srcParent = srcWidget.path.length === 0 ? srcRoot : getNodeAtPath(srcRoot, srcWidget.path);
                    ...
                    if (me?.id && movedNode.id) {
                        const dstParentNode = dstWidget.path.length === 0 ? dstRoot : getNodeAtPath(dstRoot, dstWidget.path);
    ...
    const renderComponents = () => sorted.map((comp, idx) => {
