      ...
      <Stack gap="xs">
        {items.map((item) => (
          <Group key={item.id} gap="xs" wrap="nowrap">
            <Checkbox checked={item.completed ?? false} onChange={() => toggleComplete(item.id)} />
            {editingId === item.id ? (
              <TextInput
                value={item.text}
                size="sm"
              ...
              <Text
                ...
                style={{
                  ...
                  color: item.completed
                    ? 'var(--polis-color-text-muted, var(--mantine-color-dimmed))'
                    : undefined,
                }}
            ...
            <ActionIcon variant="subtle" color="red" size="sm" onClick={() => removeItem(item.id)}>
              <IconTrash size={14} />
            </ActionIcon>
          </Group>
        ))}
