...
const TodoTaskWidget: React.FC<ComponentProps> = ({ componentId, config, onConfigChange, onDisplayUpdate }) => {
    const rootNode = (config.root as TodoTaskNode) ?? null;
    const [addType, setAddType] = useState<string | null>(null);
    const { me } = useContext(MeContext);
    const { refreshBalances, silentRefresh } = useContext(TodoContext);

    const handleUpdate = useCallback((path: number[], patch: Partial<TodoTaskNode>) => {
