...
const TodoHierarchyNav: React.FC<TodoHierarchyNavProps> = ({ page }) => {
    ...
    const findParentWeek = () => {
        ...
        if (config.todo_week_start) {
            return month.weeks.find(w => (w.config_json?.todo_week_start as string) === config.todo_week_start);
        }
        return undefined;
    };
    ...
    if (todoMonth && todoYear) {
        const monthName = new Date(todoYear, todoMonth - 1).toLocaleString('default', { month: 'long' });
        breadcrumbs.push({
            label: monthName,
        ...
        <Paper p="sm" radius="md" withBorder mb="md">
            <Group justify="space-between" wrap="nowrap">
                <Group gap="xs" wrap="nowrap">
                    ...
                    {todoLevel === 'day' && (
                        <ActionIcon variant="subtle" onClick={handleNextDay} title="Next day" disabled={isToday}>
