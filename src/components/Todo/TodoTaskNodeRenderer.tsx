...
const TodoTaskNodeRenderer: React.FC<TodoTaskNodeRendererProps> = ({
  ...
  const deepNext =
    node.task_type === 'rotating' ? getDeepNextItem(groups, cascadeRatio) : undefined;
      ...
      g.group_number === groupNum
        ? {
            ...
            children: g.children.map((item) =>
              item.id === rItemId
                ? { ...item, last_date: formatted, tally: (item.tally ?? 0) + 1 }
                : item,
            ),
          }
        : g,
  ...
  const collapsibleHeader = (
    <Group justify="space-between" mb={collapsed ? 0 : 'sm'}>
      <Group
        ...
        onClick={() => onUpdate(path, { collapsed: !collapsed })}
      >
      ...
      {group.children.map((item, idx) => {
        ...
        if (item.task_type !== 'line_item') {
          ...
          const inlineOnUpdate = (_path: number[], patch: Partial<TodoTaskNode>) => {
            ...
            const updatedGroups = groups.map((g) =>
              g.group_number === group.group_number
                ? {
                    ...g,
                    children: g.children.map((c) => (c.id === item.id ? { ...c, ...patch } : c)),
                  }
                : g,
            );
          ...
          const inlineDone = (innerPatch: Partial<TodoTaskNode>) => {
            ...
            if (
              lastGrp &&
              group.group_number === lastGrp.group_number &&
              ...
              for (let i = updatedGroups.length - 1; i >= 0; i--) {
                updatedGroups[i] = {
                  ...updatedGroups[i],
                  count_this_group: updatedGroups[i].count_this_group - sub,
                };
                sub *= cascadeRatio;
              }
            }
        ...
        return (
          ...
          >
                        ...
                        g.group_number === group.group_number
                          ? {
                              ...
                              children: g.children.map((it) =>
                                it.id === item.id ? { ...it, last_date: formatted } : it,
                              ),
                            }
                          : g,
                      ),
            ...
            <ActionIcon
              ...
              onClick={(e) => {
                e.stopPropagation();
                markDone(group.group_number, item.id);
              }}
              title="Mark done"
            >
              <IconCheck size={12} />
          ...
          {groups.map((group) => {
            ...
            return (
              <Paper
                ...
                p="xs"
                radius="sm"
                withBorder={isCurrentGroup}
                style={
                  isCurrentGroup
                    ? { borderColor: 'var(--polis-color-primary, var(--mantine-color-blue-3))' }
                    : undefined
                }
              >
                <Group
                ...
                >
                  <Group gap="xs" wrap="nowrap">
                    ...
                    <Text size="sm" fw={isCurrentGroup ? 600 : 400}>
                      {group.label}
                    </Text>
                  </Group>
                  <Group gap="xs" wrap="nowrap">
                    ...
                    {group.last_date && (
                      <Text size="xs" c="dimmed">
                        {formatLastDate(group.last_date)}
                      </Text>
                    )}
                    {group.mark_done_on_group && (
                      <ActionIcon
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          markGroupDone(group.group_number);
                        }}
                      >
                        <IconCheck size={12} />
                      </ActionIcon>
  ...
  if (isSingleGroup) {
    return (
      <Paper p={depth === 0 ? 'md' : 'xs'} radius={depth === 0 ? 'md' : 'sm'} withBorder>
        {collapsibleHeader}
        <Collapse in={!collapsed}>{renderGroupItems(groups[0], nextItemId)}</Collapse>
        {settingsDrawer}
      </Paper>
    );
  }
          ...
          {groups.map((group) => {
            ...
            return (
              ...
              >
                ...
                >
                  {expandedGroups.has(group.group_number) ? (
