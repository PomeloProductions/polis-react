        ...
        >
            <Stack gap="md" pb={120}>
                ...
                <Group gap={4} wrap="wrap">
                    ...
                    <ActionIcon variant="light" size="xs" onClick={() => updateSchedule(ALL_DAYS)} title="Select all">
                        <Text size="xs">All</Text>
                    </ActionIcon>
                    <ActionIcon variant="light" size="xs" onClick={() => updateSchedule(WORKDAYS)} title="Weekdays only">
                            ...
                            <Stack gap="xs" key={group.group_number} style={{ borderLeft: '3px solid var(--mantine-color-blue-2)', paddingLeft: 12 }}>
                                <Group gap="xs" wrap="nowrap">
                                    ...
                                    <TextInput
                                        ...
                                        onChange={(e) => {
                                            onUpdate({
                                                groups: groups.map((g, i) => i === groupIdx ? { ...g, label: e.target.value } : g),
                                            });
                                        }}
                                    />
                                    {groups.length > 1 && (
                                        <ActionIcon variant="subtle" color="red" size="xs" onClick={() => {
                                            const remaining = groups.filter((_, i) => i !== groupIdx)
                                ...
                                <Droppable droppableId={`grp-${group.group_number}`}>
                                    {(provided) => (
                                        <Stack gap="xs" ref={provided.innerRef} {...provided.droppableProps} pl="sm" style={{ minHeight: 4 }}>
                                            {group.children.map((child, childIdx) => (
                                                <Draggable key={child.id} draggableId={`settings-${child.id}`} index={childIdx}>
                                                    {(dragProvided) => (
                                                        <Group
                                                            gap="xs"
                                                            wrap="nowrap"
                                                        ...
                                                        >
                                                            ...
                                                            <TextInput
                                                                ...
                                                                onChange={(e) => {
                                                                    const newGroups = groups.map((g, gi) =>
                                                                        gi === groupIdx
                                                                            ? { ...g, children: g.children.map((c, ci) => ci === childIdx ? { ...c, label: e.target.value } : c) }
                                                                            : g
                                                                    );
                                                                    onUpdate({ groups: newGroups });
                                                                }}
                                                            ...
                                                            <Select
                                                                ...
                                                                onChange={(val) => {
                                                                    ...
                                                                    const newGroups = groups.map((g, gi) =>
                                                                        gi === groupIdx
                                                                            ? { ...g, children: g.children.map((c, ci) => ci === childIdx ? { ...c, ...patch } : c) }
                                                                            : g
                                                                    );
                                                                    onUpdate({ groups: newGroups });
                                                                }}
                                                            ...
                                                            <ActionIcon variant="subtle" color="red" size="xs" onClick={() => {
                                                                const newGroups = groups.map((g, gi) =>
                                                                    gi === groupIdx ? { ...g, children: g.children.filter((_, i) => i !== childIdx) } : g
                                                                );
                                                                onUpdate({ groups: newGroups });
                                                            }}>
                                                                <IconTrash size={12} />
                                ...
                                <Group gap="xs" pl="sm">
                                    ...
                                    />
                                    <ActionIcon variant="light" size="xs" onClick={() => {
                        ...
                        <Stack gap="xs">
                            {subItems.map((sub) => (
                                <Group key={sub.id} gap="xs" wrap="nowrap">
                                    ...
                                    />
                                    <ActionIcon variant="subtle" color="red" size="xs" onClick={() => removeSubItem(sub.id)}>
                                        <IconTrash size={12} />
                                    </ActionIcon>
                                </Group>
                            ))}
                ...
                {node.task_type === 'category' && (
                    <>
                        ...
                        {(node.children ?? []).length > 0 ? (
                                                    ...
                                                    {(dragProvided) => (
                                                        ...
                                                        >
                                                            ...
                                                            <Badge size="xs" variant="light" color={
                                                                ...
                                                                child.task_type === 'rotating' ? 'violet' : 'gray'
                                                            }>
                                                                {child.task_type === 'category' ? 'Group' :
                                                                 child.task_type === 'rotating' ? 'Rotating' : 'Item'}
                                                            </Badge>
                                                            <TextInput
                                                                value={child.label}
                                                                size="xs"
                            ...
                            <Text size="xs" c="dimmed">No children yet.</Text>
                        )}
                        <Group gap="xs">
                            <Select
