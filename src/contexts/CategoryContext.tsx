...
export const CategoryContextProvider: React.FC<PropsWithChildren<CategoryContextProviderProps>> = ({
  categoryId,
  skipCache,
  children,
}) => {
  ...
  useEffect(() => {
    if (!skipCache && cachedCategories[categoryId]) {
      ...
      CategoryRequests.getCategory(categoryId)
        .then((apiCategory) => {
        ...
        {(context) =>
          context.hasLoaded ? (
            !context.notFound ? (
              ...
              <div className={'category-not-found'}>Not Found</div>
            )
          ) : (
            <LoadingScreen />
          )
