...
export const CollectionContext =
  React.createContext<CollectionContextConsumerState>(defaultContext);
...
export const CollectionContextProvider: React.FC<
  PropsWithChildren<CollectionContextProviderProps>
> = ({ collectionId, skipCache, children }) => {
  ...
  useEffect(() => {
    if (!skipCache && cachedCollections[collectionId]) {
      ...
      CollectionManagementRequests.getCollection(collectionId)
        .then((apiCollection) => {
