 ...
 */
export interface SearchContextState<T extends BaseModel> extends BasePaginatedContextState<
  Resource<T>
> {
...
const defaultContextObject = <T extends BaseModel>(): SearchContextState<T> => ({
  ...
  expands: ['resource', 'resource.business'],
  limit: 20,
  lastSearch: null,
});
...
export const SearchContext =
  React.createContext<SearchContextState<BaseModel>>(defaultContextObject());
...
export const SearchContextProvider: React.FC<PropsWithChildren<SearchContextProviderProps>> = ({
  latitude,
  longitude,
  searchText,
  children,
}) => {
  const [searchState, setSearchState] = useState<SearchContextState<BaseModel>>({
    ...defaultContextObject(),
  });

  const wrappedSetSearchStateForBaseContext: Dispatch<
    SetStateAction<BasePaginatedContextState<Resource<BaseModel>>>
  > = useCallback(
    (action) => {
      setSearchState((currentSearchState) => {
        const baseStateChanges =
          typeof action === 'function'
            ? (
                action as (
                  prevState: BasePaginatedContextState<Resource<BaseModel>>,
                ) => BasePaginatedContextState<Resource<BaseModel>>
              )(currentSearchState)
            : action;
  ...
  useEffect(() => {
    setSearchState((prevState) => {
      ...
      const contextWithNewSettings = createCallbacks<Resource<BaseModel>>(
        wrappedSetSearchStateForBaseContext,
        nextState,
        '/resources',
      );
  ...
  const fullContext = {
    ...searchState,
    ...createCallbacks<Resource<BaseModel>>(
      wrappedSetSearchStateForBaseContext,
      searchState,
      '/resources',
    ),
