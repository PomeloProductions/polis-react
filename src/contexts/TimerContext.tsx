...
import { getRunningTimer, startRunningTimer, stopRunningTimer } from '../services/requests/TodoRequests';
...
interface TimerContextValue {
    ...
    registerOnStop: (componentId: number, itemId: string, onStop: (elapsedHours: number) => void, meta?: { sessionBudgetHours?: number; preLoggedHours?: number; balanceHours?: number; dailyTargetHours?: number; todoBalanceId?: number; budgetHours?: number; onUpdateBalance?: (v: number) => void }) => void;
...
export const TimerContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const lastSessionRef = useRef<{ componentId: number; itemId: string; sessionElapsedSeconds: number } | null>(null);
    ...
    const totalTodaySeconds = useMemo(() => {
        ...
        const now = new Date();
        const sameDay = startDate.getFullYear() === now.getFullYear()
    ...
    const labelCurrentSession = useCallback((label: string) => {
        if (!running) return;
        setRunning((prev) => {
            if (!prev) return prev;
    ...
    const registerOnStop = useCallback(
        (componentId: number, itemId: string, onStop: (elapsedHours: number) => void, meta?: { sessionBudgetHours?: number; preLoggedHours?: number; balanceHours?: number; dailyTargetHours?: number; todoBalanceId?: number; budgetHours?: number; onUpdateBalance?: (v: number) => void }) => {
    ...
    const stopTimer = useCallback(() => {
        ...
        setRunning(null);
        if (me?.id) {
            stopRunningTimer(me.id, totalSession != null ? { session_elapsed_seconds: totalSession } : undefined)
                .then(() => { afterStop?.(); })
    ...
    const startTimer = useCallback(
        (target: TimerTarget) => {
            ...
            const sessionElapsed = (prior
                && prior.componentId === target.componentId
                && prior.itemId === target.itemId)
    ...
    return (
        <TimerContext.Provider
            value={{
                activeTimer, elapsedSeconds, sessionSeconds, totalTodaySeconds, sessions,
                startTimer, stopTimer, resetSession, isTracking, registerOnStop,
                registerAfterStop: useCallback((cb: (() => void) | null) => { onAfterStopRef.current = cb; }, []),
                updateStartTime, labelCurrentSession,
            }}
        >
