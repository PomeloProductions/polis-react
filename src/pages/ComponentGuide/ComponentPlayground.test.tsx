...
jest.mock('../../components/PageRenderer/ComponentRegistry', () => {
  const React = jest.requireActual('react');
  const Stub: React.FC = () => React.createElement('div', { 'data-testid': 'rendered-widget' });
  return {
    __esModule: true,
...
describe('ComponentPlayground', () => {
  ...
  test('shows sign-in alert when not logged in', () => {
    renderAt('/component-guide/playground/stats_cards');
    expect(screen.getByText(/Sign in to render live component previews/i)).toBeInTheDocument();
  });
