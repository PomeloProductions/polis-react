...
import { IconFlask, IconPlus, IconInfoCircle, IconSettings } from '@tabler/icons-react';
...
const GenericConfigDrawer: React.FC<GenericConfigDrawerProps> = ({
  ...
  return (
    ...
    >
      <Stack gap="md">
        <ConfigEditor options={entry.configOptions} config={config} onChange={onChange} />
      </Stack>
    </Drawer>
  );
};
...
const ComponentPlayground: React.FC = () => {
  ...
  const [tabConfigs, setTabConfigs] = useState<Record<string, Record<string, unknown>>>(() => {
    if (!entry) return {};
    return Object.fromEntries(entry.exampleConfigs.map((ex, i) => [String(i), ex.config]));
  });
                ...
                {hasDrawer && (
                  <Group justify="flex-end" mb="md">
                    <Tooltip label="Configure" position="left">
                      <ActionIcon variant="subtle" color="gray" onClick={() => setDrawerOpen(true)}>
