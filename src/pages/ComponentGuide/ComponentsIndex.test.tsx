...
describe('ComponentsIndex', () => {
  ...
  test('filters entries by the search input', () => {
    ...
    const match = COMPONENT_GUIDE.find((e) => e.displayName.toLowerCase().includes('stats'));
    if (match) {
      expect(screen.getByText(match.displayName)).toBeInTheDocument();
    }
    // Other entries that don't match should disappear
    const nonMatch = COMPONENT_GUIDE.find((e) => !e.displayName.toLowerCase().includes('stats'));
    if (nonMatch) {
