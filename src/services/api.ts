...
export const responseErrorInterceptor = (error: AxiosError): Promise<AxiosResponse | never> => {
	...
	if (error.response?.status === 401 && error.config) {
		...
		releaseSlot();
		if (appState) appState.dispatch(decrementLoadingCount());
		const badToken = (error.config?.headers?.['Authorization'] as string | undefined)?.replace('Bearer ', '');
	...
	if (error.response?.status === 429 && error.config) {
		const retryCount = ((error.config as unknown as Record<string, unknown>)._retryCount as number) || 0;
 ...
 */
export const dedupedGet = <T = unknown>(url: string, config?: Record<string, unknown>): Promise<AxiosResponse<T>> => {
